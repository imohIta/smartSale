...
class Database extends \FuniObject{
    ...
    public function newConnection( Array $options ){
		 ...
		 $host = isset($options['host']) ? $options['host'] : $registry->get('config')->get('dbHost');
		 $user = isset($options['user']) ? $options['user'] : $registry->get('config')->get('dbUser');
		 $pwd = isset($options['password']) ? $options['password'] : $registry->get('config')->get('dbPwd');
		 $dbName = isset($options['dbName']) ? $options['dbName'] : $registry->get('config')->get('dbName');
    ...
    public function setActiveConnection( $connectionName ){
        $this->_activeConnection = $connectionName;
